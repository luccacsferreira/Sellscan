...
export function ScanDashboard({
  ...
  const [analysisHistory, setAnalysisHistory] = useState<ProductAnalysis[]>([
    scan.analysis,
  ]);
  ...
  const analysis = useMemo(
    () => ({
      ...
      worthRange: scan.analysis.worthRange ||
        scan.analysis.priceRange || { min: 0, max: 0, sweetSpot: 0 },
      priceRange: scan.analysis.priceRange || {
        min: 0,
        max: 0,
        sweetSpot: 0,
        platformAverage: 0,
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      onMessageSent?.();
      const response = await chatAboutProduct(
      ...
      setChatMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            "I encountered an error syncronizing with the pricing engine. Please attempt the request again.",
        },
      ]);
    } finally {
      setIsSending(false);
  ...
  const handleRevert = (index: number) => {
    if (analysisHistory[index]) {
      ...
      setChatMessages((prev) => [
        ...prev,
        { role: "assistant", content: `Reverted to revision ${index + 1}.` },
      ]);
    }
  };
  ...
  return (
    <div className="pt-20 pb-28 md:pb-20 px-4 md:px-10 max-w-[1400px] mx-auto min-h-screen flex flex-col relative space-y-6">
      <AnimatePresence>
        {selectedMockup && (
          <PlatformMockup
        ...
        <div className="relative">
          ...
          <AnimatePresence>
            {isProjectMenuOpen && (
              ...
              >
                <p className="text-[9px] font-black text-brand-text-muted uppercase tracking-[0.2em] px-4 py-3 border-b border-brand-border/50 mb-1">
                  Select Destination
                </p>
          ...
          {animationStage >= 1 && (
            <div ref={box2Ref} className="px-1 md:px-0">
              <IdentityBox
                ...
                onComplete={() =>
                  setAnimationStage((prev) => Math.max(prev, 2))
                }
              />
            </div>
          )}
          ...
          {animationStage >= 2 && (
            <div ref={box3Ref}>
              <PriceInsightBox
                ...
                onComplete={() =>
                  setAnimationStage((prev) => Math.max(prev, 3))
                }
              />
            </div>
          )}
          ...
          {animationStage >= 4 && (
            <div ref={box5Ref}>
              <PracticalTipsBox
                ...
                onComplete={() =>
                  setAnimationStage((prev) => Math.max(prev, 5))
                }
              />
            </div>
          )}
          ...
          {animationStage >= 5 && (
            <div ref={box6Ref}>
              <MarketSentimentBox
                ...
                onComplete={() =>
                  setAnimationStage((prev) => Math.max(prev, 6))
                }
              />
            </div>
          )}
